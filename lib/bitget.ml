...
let place_order_with ~api_key ~secret ~passphrase ~symbol ~side ~orderType ~force ~price ~quantity =
  ...
  let body_list =
    if price <> "" then ("price", `String price) :: base else base
  in
  let body = Yojson.Safe.to_string (`Assoc body_list) in
  let headers =
...
let place_limit_order ~api_key ~secret ~passphrase ~symbol ~side ~price ~quantity =
  place_order_with ~api_key ~secret ~passphrase ~symbol ~side
    ~orderType:"limit" ~force:"post_only" ~price ~quantity

let place_market_order ~api_key ~secret ~passphrase ~symbol ~side ~quantity =
  place_order_with ~api_key ~secret ~passphrase ~symbol ~side
    ~orderType:"market" ~force:"normal" ~price:"" ~quantity
