(* Phase 8: Final Validation + Asymmetric Thresholds *)
(* Fine-grained search around w=500, th=1.0, EMA200 *)
...
let () =
  ...
  Printf.printf "=== Phase 8: Final Validation + Asymmetric Thresholds ===\n\n";
  ...
  (* Precompute z-scores for fine-grained windows *)
  Printf.printf "Precomputing z-scores...\n";
  let windows = [300; 350; 400; 450; 500; 550; 600; 700] in
  ...
  let trend_configs = [
    ("EMA150", compute_ema prices 150);
    ("EMA200", compute_ema prices 200);
    ("EMA250", compute_ema prices 250);
    ("SMA200", compute_sma prices 200);
    ("SMA300", compute_sma prices 300);
  ] in

  Printf.printf "Running fine-grained search...\n\n";

  let thresholds = [0.5; 0.7; 0.8; 0.9; 1.0; 1.1; 1.2; 1.3; 1.5] in
  ...
  List.iter (fun (w, z) ->
    List.iter (fun th ->
      List.iter (fun (tname, trend) ->
        ...
        in
        results := (w, th, th, tname, trades, wr, pf, mdd, ret) :: !results
      ) trend_configs;
      (* No trend *)
      count := !count + 1;
      let signals = Array.init n (fun i ->
      ...
      let trades, wr, pf, mdd, ret =
        run_with_signals oos_signals oos_slice 0.001 0.5 0.30 0.15
      in
      results := (w, th, th, "None", trades, wr, pf, mdd, ret) :: !results
    ) thresholds
  ) zscores;

  (* Asymmetric thresholds: buy_th != sell_th *)
  Printf.printf "Running asymmetric threshold search...\n\n";
  let buy_thresholds = [0.5; 0.7; 0.8; 0.9; 1.0; 1.2] in
  let sell_thresholds = [0.5; 0.7; 0.8; 0.9; 1.0; 1.2; 1.5] in
  let asym_total = List.length windows * List.length buy_thresholds * List.length sell_thresholds * 2 in
  let asym_count = ref 0 in

  List.iter (fun (w, z) ->
    List.iter (fun buy_th ->
      List.iter (fun sell_th ->
        (* Skip symmetric combos (already tested) *)
        if buy_th <> sell_th then begin
          asym_count := !asym_count + 1;
          if !asym_count mod 20 = 0 then
            Printf.printf "Asym Progress: %d/%d\n" !asym_count asym_total;
          (* With EMA200 *)
          let ema200 = List.assoc "EMA200" trend_configs in
          let signals = Array.init n (fun i ->
            if prices.(i) > ema200.(i) then
              if z.(i) < -.buy_th then Buy else Hold
            else
              if z.(i) > sell_th then Sell else Hold
          ) in
          let oos_name, oos_start, oos_end = List.nth periods 3 in
          let oos_slice = slice_by_timestamp all_candles oos_start oos_end in
          let oos_signals = slice_signals signals all_candles oos_start oos_end in
          let trades, wr, pf, mdd, ret =
            run_with_signals oos_signals oos_slice 0.001 0.5 0.30 0.15
          in
          results := (w, buy_th, sell_th, "EMA200", trades, wr, pf, mdd, ret) :: !results;
          (* No trend *)
          let signals2 = Array.init n (fun i ->
            if z.(i) < -.buy_th then Buy
            else if z.(i) > sell_th then Sell
            else Hold
          ) in
          let oos_signals2 = slice_signals signals2 all_candles oos_start oos_end in
          let trades2, wr2, pf2, mdd2, ret2 =
            run_with_signals oos_signals2 oos_slice 0.001 0.5 0.30 0.15
          in
          results := (w, buy_th, sell_th, "None", trades2, wr2, pf2, mdd2, ret2) :: !results
        end
      ) sell_thresholds
    ) buy_thresholds
  ) zscores;

  let sorted = List.sort (fun (_, _, _, _, _, _, _, _, r1) (_, _, _, _, _, _, _, _, r2) ->
    compare r2 r1
  ) !results in

  Printf.printf "\n=== Top 30 Configs (sorted by OOS return) ===\n\n";
  Printf.printf "Rank  W    BTh  STh  Trend   trades win%%   pf      max_dd     OOS%%\n";
  Printf.printf "----- ---- ---- ---- ------- ------ ------ ------- ---------- ----------\n";

  let rec show_top n = function
    | [] -> ()
    | _ when n = 0 -> ()
    | (w, bth, sth, tname, trades, wr, pf, mdd, ret) :: rest ->
      let rank = 30 - n in
      let pf_str = if pf = Float.infinity then "inf" else Printf.sprintf "%.2f" pf in
      let asym = if bth <> sth then "*" else " " in
      Printf.printf "%-5d %-4d %.1f  %.1f  %-7s %-6d %-5.1f%% %-7s %-9.2f%% %+.2f%%\n"
        rank w bth sth (tname ^ asym) trades wr pf_str mdd ret;
      show_top (n - 1) rest
  in
  show_top 30 sorted;
  ...
  Printf.printf "=== Best Config Deep Dive (all periods) ===\n\n";
  let best_w, best_bth, best_sth, best_tname =
    match sorted with
    | (w, bth, sth, tname, _, _, _, _, _) :: _ -> (w, bth, sth, tname)
    | [] -> (500, 1.0, 1.0, "EMA200")
  in
  Printf.printf "Best config: w=%d, buy_th=%.1f, sell_th=%.1f, trend=%s\n\n"
    best_w best_bth best_sth best_tname;

  let best_z = List.assoc best_w zscores in
  let signals =
    if best_tname = "None" then
      Array.init n (fun i ->
        if best_z.(i) < -.best_bth then Buy
        else if best_z.(i) > best_sth then Sell
        else Hold
      )
    else
      ...
      Array.init n (fun i ->
        if prices.(i) > trend.(i) then
          if best_z.(i) < -.best_bth then Buy else Hold
        else
          if best_z.(i) > best_sth then Sell else Hold
      )
  in
