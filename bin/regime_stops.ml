(* Phase 2.5: Regime + Stop Loss / Trailing Stop *)
(* Tests if risk management can reduce the 80%+ drawdowns *)
...
(* Apply trade with optional stop loss and trailing stop *)
let apply_trade_with_stops pf signal price fee stop_loss_pct trailing_stop_pct =
  match signal with
  | Buy when pf.position = 0.0 ->
    let pos = pf.cash *. (1.0 -. fee) /. price in
    { pf with cash = 0.0; position = pos; entry_price = price;
      highest_price_since_entry = price }
  | Sell when pf.position > 0.0 ->
    let revenue = pf.position *. price *. (1.0 -. fee) in
    let pnl = revenue -. (pf.position *. pf.entry_price) in
    if pnl > 0.0 then
      { pf with cash = revenue; position = 0.0;
        trades = pf.trades + 1; wins = pf.wins + 1;
        total_won = pf.total_won +. pnl;
        entry_price = 0.0; highest_price_since_entry = 0.0 }
    else
      { pf with cash = revenue; position = 0.0;
        trades = pf.trades + 1;
  ...
  | _ ->
    ...
    if pf.position > 0.0 then
      ...
      let drop_from_high = (pf.highest_price_since_entry -. price) /. pf.highest_price_since_entry in
      let stop_triggered =
        (stop_loss_pct > 0.0 && drop_from_entry >= stop_loss_pct) ||
        (trailing_stop_pct > 0.0 && drop_from_high >= trailing_stop_pct)
      in
      if stop_triggered then
        ...
        let new_high = max pf.highest_price_since_entry price in
        { pf with highest_price_since_entry = new_high }
    else pf

let backtest_with_stops candles adx_threshold imb_ema_period buy_threshold
    sell_threshold fee stop_loss_pct trailing_stop_pct =
  let n = Array.length candles in
  ...
  for i = 200 to n - 1 do
    ...
    pf := apply_trade_with_stops !pf signal price fee stop_loss_pct trailing_stop_pct;
...
let () =
  ...
  (* Best base config: ADX threshold 10, Range-Only, EMA 15, buy=0.3, sell=0.3 *)
  (* Test stop loss levels *)
  Printf.printf "\n=== ADX Range-Only + Stop Loss (no trailing) ===\n";
  Printf.printf "%-8s %-6s %-6s %-7s %-9s  %s\n%!"
    "stop%" "trades" "win%" "pf" "max_dd" "return";
  List.iter (fun sl ->
    let trades, wr, pf, mdd, ret =
      backtest_with_stops candles 10.0 15 0.3 0.3 fee sl 0.0 in
    Printf.printf "%-8.1f %-6d %-5.1f%% %-7.2f %-7.2f%% %+.2f%%\n%!"
      (sl *. 100.0) trades wr pf mdd ret
  ) [0.0; 0.02; 0.03; 0.05; 0.08; 0.10; 0.15; 0.20];

  (* Test trailing stop levels *)
  Printf.printf "\n=== ADX Range-Only + Trailing Stop (no fixed stop) ===\n";
  Printf.printf "%-8s %-6s %-6s %-7s %-9s  %s\n%!"
    "trail%" "trades" "win%" "pf" "max_dd" "return";
  List.iter (fun ts ->
    let trades, wr, pf, mdd, ret =
      backtest_with_stops candles 10.0 15 0.3 0.3 fee 0.0 ts in
    Printf.printf "%-8.1f %-6d %-5.1f%% %-7.2f %-7.2f%% %+.2f%%\n%!"
      (ts *. 100.0) trades wr pf mdd ret
  ) [0.0; 0.02; 0.03; 0.05; 0.08; 0.10; 0.15; 0.20];

  (* Combined stop + trailing *)
  Printf.printf "\n=== ADX Range-Only + Stop + Trailing ===\n";
  Printf.printf "%-8s %-8s %-6s %-6s %-7s %-9s  %s\n%!"
    "stop%" "trail%" "trades" "win%" "pf" "max_dd" "return";
  List.iter (fun sl ->
    List.iter (fun ts ->
      let trades, wr, pf, mdd, ret =
        backtest_with_stops candles 10.0 15 0.3 0.3 fee sl ts in
      Printf.printf "%-8.1f %-8.1f %-6d %-5.1f%% %-7.2f %-7.2f%% %+.2f%%\n%!"
        (sl *. 100.0) (ts *. 100.0) trades wr pf mdd ret
    ) [0.03; 0.05; 0.10]
  ) [0.05; 0.10; 0.15];

  (* ADX 12 Range-Only with best stops *)
  Printf.printf "\n=== ADX 12 Range-Only + Stop Loss ===\n";
  Printf.printf "%-8s %-6s %-6s %-7s %-9s  %s\n%!"
    "stop%" "trades" "win%" "pf" "max_dd" "return";
  List.iter (fun sl ->
    let trades, wr, pf, mdd, ret =
      backtest_with_stops candles 12.0 15 0.3 0.3 fee sl 0.0 in
    Printf.printf "%-8.1f %-6d %-5.1f%% %-7.2f %-7.2f%% %+.2f%%\n%!"
      (sl *. 100.0) trades wr pf mdd ret
  ) [0.0; 0.03; 0.05; 0.10; 0.15];

  (* ADX 15 Range-Only with best stops *)
  Printf.printf "\n=== ADX 15 Range-Only + Stop Loss ===\n";
  Printf.printf "%-8s %-6s %-6s %-7s %-9s  %s\n%!"
    "stop%" "trades" "win%" "pf" "max_dd" "return";
  List.iter (fun sl ->
    let trades, wr, pf, mdd, ret =
      backtest_with_stops candles 15.0 15 0.3 0.3 fee sl 0.0 in
    Printf.printf "%-8.1f %-6d %-5.1f%% %-7.2f %-7.2f%% %+.2f%%\n%!"
      (sl *. 100.0) trades wr pf mdd ret
  ) [0.0; 0.03; 0.05; 0.10; 0.15]
